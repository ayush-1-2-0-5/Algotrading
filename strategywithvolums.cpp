...
map<string, vector<pair<bool, string>>> fun4(
    ...
    for (const auto& entry : topperccentstocksdatewise) {
        const string& currdate = entry.first;
        futuredates[currdate] = find_futuredate(mappp, currdate);
        futuredates2[currdate] = make_pair(
            find_futuredate2(mappp, currdate),
            find_futuredate3(mappp, currdate)
        );
    }
    for (const auto& entry : topperccentstocksdatewise) {
        const string& date = entry.first;
        const vector<pair<double, string>>& stocksList = entry.second;

        int i1 = -1, j1 = -1;
        const pair<string, string>& datess = futuredates2.at(date);
        ...
        const string& futureDate = futuredates.at(date);
        ...
        for (const auto& stockPair : stocksList) {
            const string& stockName = stockPair.second;
            ...
            if (itr != stocks.end()) {
                const auto& prices = itr->prices;
                const auto& ema = itr->ema;
                const auto& ath = itr->ath;
                const auto& priceintovol = itr->priceintovolume;

                long long avg = findavgpriceintovol(priceintovol, i1, j1);

                bool cond = (prices[j].second > ema[j].second &&
                             prices[j].second >= ath[j].second * criteriaforath / 100 &&
                             avg >= 10000001);
    ...
    for (const auto& entry : answer) {
        ...
        for (const auto& stockPair : entry.second) {
            cout << "(" << stockPair.second << ", " << stockPair.first << ") ";
        }
        cout << "\n";
    }
...
void CreateFileForRegression(const std::vector<std::pair<std::string, double>>& portfolioreturns, 
                             const std::string& outputPath = "./monthly_portfolio_for_regression.csv", 
                             int start = 0, int end = -1)
{
    string filenameforregression = outputPath;
    ofstream outFile(filenameforregression);

    if (!outFile.is_open()) {
        cerr << "Error: Unable to create or open the file at path: " << filenameforregression << endl;
        return;
    }
    outFile << "Date,Return (%)\n";


    if (end == -1 || end > portfolioreturns.size()) {
        end = portfolioreturns.size();
    }
    for (int j = start; j < end; ++j) {
        const auto& entry = portfolioreturns[j];
        const std::string& date = entry.first;
        double returnPercentage = entry.second;
        outFile << date << "," << returnPercentage << "\n";
    }
    outFile.close();
    cout << "CSV file '" << filenameforregression << "' created successfully." << endl;
...
) 
{
    ...
    cout << "Enter 1 to create a different CSV file\nEnter 2 to update the portfolio_returns file for Power BI\n 3: file for regresstion testing" << endl;
    int n = -1;
    cin >> n;
    if (n != 1 && n != 2) {
